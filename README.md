...
| blocked   | `!`   | orange     | a permission prompt or question is waiting on you    |
...
(blocked > done > working). `blocked` always shows through a wait or park.

Sounds play for blocked and done, only when the window is not on screen.

## Keys (all `prefix +`, configurable)

| Key | Action |
|-----|--------|
| `N` | go to the next blocked window, else the next done window |
...
session (asks first), `ctrl-w` wait, `ctrl-p` park, `esc` quit.
...
| `Notification` permission_prompt, elicitation, agent_needs_input | blocked |
| `Notification` idle_prompt                    | done (covers turns ended with Esc, which skip `Stop`) |
...
```sh
agent-signal set working|blocked|done [pane-id]
...
urgent pane. Pane options inherit from the window in tmux formats, so the
script reads them with `show-option -p` rather than through formats.
